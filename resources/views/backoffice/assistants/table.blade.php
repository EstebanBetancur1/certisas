            ...
            @foreach($items as $item)
                <tr>
                        ...
                        @if($i === 'status')
                            <td class="text-center">
                                ...
                                <div class="btn-group" role="group">
                                    <button class="btn btn-sm @if((int)$item->company_user_status === 0)  btn-danger @elseif((int)$item->company_user_status === 1) btn-success @else btn-default  @endif btn-xs dropdown-toggle" type="button" id="dropdownMenu-{{ $i }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
                                        {{ (array_key_exists($item->company_user_status, $statusOptions)) ? $statusOptions[$item->company_user_status] : $field }}
                    ...
                    <td class="text-center">
                        ...
                        <div class="btn-group">
                            <a href="{{ route($routeEdit, ['id' => $item->id]) }}" class="btn btn-secondary btn-sm"><i class="fa fa-eye"></i></a>
                            @method('DELETE')
